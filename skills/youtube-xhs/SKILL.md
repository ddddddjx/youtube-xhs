...
按 `templates/video_post.md` 写 `视频版/标题.txt`（一行，≤20 字，先过「标题五条铁律」）和 `视频版/正文.txt`：钩子、**时间轴**、3 条要点、「我的看法」（可被反对的判断，写成完整句子，不留占位）、二选一互动问题、下一期预告、原视频出处、标签（≤8 个）。另写 `视频版/开头卡.txt`（见步骤 2）和 `视频版/置顶评论.txt`（见步骤 6 第 9 步）。
...
5. 写 `图文版/标题.txt` 和 `图文版/正文.txt`（罗尼轨：新闻句标题 → 机构日期开篇 → 结论 → 我的看法 → 二选一互动问题 → 下一期预告 → 出处与截图版权说明 → 标签 ≤8 个：3 个大流量 + 3–5 个精准），同样纯文本、可直接粘贴
...
复盘结论：前五篇笔记封面点击率 5–12%，同类中位数 19.9%；收藏 59、评论 1、涨粉 3。卡在三处：封面看不清 → 止步首轮推荐；互动问题太开放 → 没评论；只转述没立场 → 读者记住了信源没记住 Krypto。目标：2 周内点击率 ≥15%（4 周 25%），每篇评论 ≥5、涨粉 ≥5，有一篇曝光破 5k。全文在 yt2red 仓库 `复盘/2026-09-22_五篇笔记复盘.md`。

**封面规则（视频版和图文版都要出三版）**
- 封面只放一句 8–12 字的结论，字号占画面宽度 80% 以上；小字、图表、来源角标全部下沉到第 2 页
...
**互动与转粉规则**
- 互动问题固定二选一：「你是 A 还是 B，评论区打 A 或 B」，不用「你怎么看」这类开放式提问
- 标题或封面眉标带系列号「海外精读 #N」，末页固定「下一期：…」预告
- 图文末页名片卡：Krypto说AI + 一句定位 + 关注看下一期 + 扣 1 领模板
- 每篇必有一个可领取物，写进 `置顶评论.txt`；发布后 1 小时内置顶这条评论，回复每一条评论（首轮推荐池看前几小时的互动密度）
- 标签 ≤8 个：3 个大流量 + 3–5 个精准
- 发布时段固定（工作日 12:00 或 21:00 二选一），连续 2 周不换，方便对比

**dbskill 的行动顺序**：先有可领取物 → 再改封面 → 标题过铁律 → 开头 3 秒 → 我的看法有立场。
...
- 每回填 5 篇对照「硬规则」里的目标看一次：点击率是否 ≥15% 并向 25% 靠近、评论是否 ≥5、涨粉是否 ≥5；未达标的项回到对应规则修
